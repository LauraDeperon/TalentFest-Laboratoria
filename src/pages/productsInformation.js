  ...
  return (
    <>
      ...
      <div>
        ...
        <p className='about-product'>{itemDescription}</p>
        <p className='product-titles'>Como é feito</p>
        <Splide className='carousel-div'>
          {itemProcesses.map((process, index) => (
            <SplideSlide className='carousel-container'>
              <p className='carousel' key={index}>
                {process}
              </p>
            </SplideSlide>
          ))}
        ...
        <video src={itemRecipe} controls type='video/mp4' className='video' />
      </div>
    </>
  );
}
